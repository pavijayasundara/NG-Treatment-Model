function fittedVals= truncated_fit_simultaneouslyBetaLactams(NGVals,antibioticConcVals,~,phi_max,phi_minIntGuess)

thours=[0;1;2;3;4;5;6];
phi_maxVal=phi_max;
conc=antibioticConcVals(2:end-3);

X=[phi_maxVal;conc;NGVals(2,2)];%concentrations and phi_max value
%cocnvals should be only the 10 concentrations excluding the 0 and maximum
%concentrations

Y=NGVals(2:end-3,2:end);

%finding the values which are=100 and replacing them with NaN.
if find(Y==100,1)>0
        Y(find(Y==100))=NaN;
       
end

%parameters estimated: MIC, Hill_k, phi_min
...
function output= model(xEstimate,thours,X)
    ...
    intVals=repmat(X(end),8,1);%initial value of the ODE system
