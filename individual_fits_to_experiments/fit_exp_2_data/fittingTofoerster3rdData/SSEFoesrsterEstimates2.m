function [SSEconc,totalSSE]= SSEFoesrsterEstimates2(NGVals,antibioticConcVals,titleName,phi_max,phi_min,MIC,Hill_k)

thours=[0;1;2;3;4;5;6];

%cocnvals should be only the 10 concnentratons excluding the 0 and maximum
%concentrations
concVals=round(antibioticConcVals(2:end),5);
...
colourVals={[75,0,13]./255,[176,224,230]./255,[0,191,25]./255,[30,144,255]./255,[100,149,237]./255,...
    [70,130,180]./255,[95,158,160]./255,[123,104,238]./255,[72,61,139]./255,[0,0,255]./255,[0,0,128]./255};

fittedValues=model([MIC;Hill_k],thours,[phi_max;phi_min;antibioticConcVals(2:end);NGVals(2,2)]);
...
totalSSE=sum(SSEconc);

plotHandles = zeros(1,11);
plotLabels = cell(1,11);
...
function output= model(xEstimate,thours,X)
    ...
    intVals=repmat(X(end),11,1);%initial value of the ODE system
    ...
    function s=Equations(~,y,para)
        ...
        MIC=xEstimate(1);
        Hill_k=xEstimate(2);
        conc=para(3:end,1);%concentrations
