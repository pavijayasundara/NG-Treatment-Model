function [SSEconc,totalSSE]= SSEFoesrsterCipro(NGVals,antibioticConcVals,titleName,phi_max,phi_min,MIC,Hillk)

thours=[0;1;2;3;4;5;6];
phi_maxVal=phi_max;
conc=antibioticConcVals(2:end-5);
...
fittedValues=model([MIC,Hillk,phi_min],thours,X);%columns are time points, rows are concentrations 
...
function output= model(xEstimate,thours,X)
    ...
    intVals=repmat(X(end),6,1);%initial value of the ODE system
