function [SSEconc,totalSSE]= SSEFoesrsterPenicillin(NGVals,antibioticConcVals,titleName,phi_max,phi_min,MIC,Hillk)
...
plotHandles = zeros(1,10);
plotLabels = cell(1,10);
for k=1:length(conc)
...
function output= model(xEstimate,thours,X)
    ...
    intVals=repmat(X(end),9,1);%initial value of the ODE system
